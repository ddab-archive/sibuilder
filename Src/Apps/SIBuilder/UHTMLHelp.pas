 ...
 * UHTMLHelp.pas
 *
 * Provides a partial translation of HTMLHelp header files for use in SIBuilder.
 * Dynamically imports HTMLHelp function from hhctrl.ocx.
 *
...
function HtmlHelp(hwndCaller: HWND; pszFile: LPCTSTR; uCommand: UINT;
  ...
  specified command. hwndCaller is the handle of the window calling HtmlHelp.
  If notifcations are enabled HtmlHelp sends notification messages to this
  window. pszFile specifies either a .chm file or a topic file within a help
  file, depending uCommand. May also specify a window. uCommand specifies a
  command to be executed. dwData specifies any data that may be required, based
  on the value of the uCommand parameter. Depending on uCommand and the result,
  either the handle of the help window or 0 are returned. Zero can indicate
  either failure or that the help window has not yet been created}
...
type
  ...
  {
  EHtmlHelp:
    Exception raised when there are problems accessing HTML help via the API.
  }
...
// Try to initialise HTML help library
LoadHtmlHelp;
